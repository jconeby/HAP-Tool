...
"""
The /var/log/boot.log file keeps records of system startup messages.  Look for any unexpected behavior during
the boot process. If there are unfamiliar kernel modules loading during boot, it might be a sign of kernel level compromise.
If there are frequent or unexpected service failures, this can be indicative of tampering.  A rootkit or malicious process
might cause legitimate services to fail to start properly.
"""
...
def create_index_pattern(elastic_url, elastic_user, elastic_pass, index_pattern, time_field_name):
